...
export const Facture = () => {
    ...
    //FOr Edit Model
    const [ViewShowFR, SetShowFR] = useState(false)
    const handleShowFR = () => { SetShowFR(true) }
    const hanldeCloseFR = () => { SetShowFR(false) }
...
/********************************************************************************************************/
    ...
    const GetFournisseurData = (id)=>{
        //here we will get all employee data
        const url = `http://localhost:5000/getFournisseurBYid/${id}`
        axios.get(url)
            .then(response => {
                const result = response.data;
                setNom_prenom(result[0].Nom_prenom)
                setAdresse(result[0].adresse)
                setTéléphone(result[0].téléphone)
                setMat_fisc(result[0].mat_fisc)
...
<div style={{marginLeft:"100px"}}>
    ...
    const PaiementAdd = (RowData) => {
        ...
        axios.post(url,{
            ...
            IDFR:IDFR,
            fichier:RowData.Fichier,
        TypeFichier:RowData.TypeFichier})
        }
                        ...
                        <tbody >
                            {Data.map((item) =>
                                ...
                                <tr key={item._id} style={{overlay:"red"}}>
                                    <td style={{color:'#048a93',hover:{backgroundColor:"red"}}}><h6 style={{cursor:"pointer",hover:"text-decoration: underline"}} onClick={() => { handleViewShow(SetRowData(item),setEtat(item.Etat),setId(item.id),setidfact("F"+item.id),setDateFact(item.DateFact),setDateAjout(item.DateAjout),setNumFact(item.numfact),setFournisseur(item.Fournisseur),setIDFR(item.IDFR)) }}>F{item.id}</h6></td>
                                    

                                    <td>{item.numfact}</td>
                                    <td>{item.DateFact}</td>
                                    <td style={{cursor:"pointer",hover:"text-decoration: underline",color:'#048a93'}} onClick={()=> {handleShowFR(GetFournisseurData(item.IDFR))}}><h6>{item.Fournisseur}</h6></td>
...
<Table>
  <tr><td style={{width:"323px"}}><b>Montant</b></td>
                           ...
                           <tr><td>     <Button style={{backgroundColor:"#048a93",marginLeft:"300px",width:"200px"}} onClick={()=>PaiementAdd(RowData)} >Payer</Button>
</td></tr>
                            </Table>
                            </div>
                ...
                >
                    ...
                    <Modal.Body>
                        ...
                        <b>Information générale</b>
                            ...
                            <td> 
                                <label>Nom</label>
                                <input type="text" className='form-control'  defaultValue={Nom_prenom} readOnly/>
                            </td>
                            <td > 
                                <label>Email</label>
                            ...
                            <td> 
                                ...
                                <input type="text" className='form-control'  defaultValue={téléphone}readOnly/>
                            </td>
                            
                                
                            <td>                     
                                <label>Addresse</label>
                                <input type="text" className='form-control'  defaultValue={adresse}readOnly/>
                                </td></tr>
                                <tr><td>                     
                                <label>Matricule fiscal</label>
                                <input type="text" className='form-control'  defaultValue={mat_fisc}readOnly/>
                                </td>
                                </tr>
                    ...
                    <Modal.Footer>
                        <Button variant='secondary' onClick={hanldeCloseFR}>Fermer</Button>
                    </Modal.Footer>
                </Modal>
